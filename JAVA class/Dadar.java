...
public class Dadar implements ActionListener
{
	DataSource source;
	Instances dataset;
	Instance crimeRate;
	...
	JFrame F = new JFrame("Dadar and Nagar");
	...
	{
		source = new DataSource("C:\\Users\\Chaitanya\\\\Desktop\\\\States\\\\Dadar&Nagar.arff");
		...
		rate1 = (Math.abs((dataset.instance(5).value(1)-crime)/(dataset.instance(5).value(1)))*100);
		rate2 = (Math.abs((dataset.instance(6).value(1)-crime)/(dataset.instance(6).value(1)))*100);
		rate3 = (Math.abs((dataset.instance(7).value(1)-crime)/(dataset.instance(7).value(1)))*100);
	}
	...
	void input()
	{
		...
		UT = new Label("Dadar and Nagar",Label.CENTER);
		UT.setFont(Q);
